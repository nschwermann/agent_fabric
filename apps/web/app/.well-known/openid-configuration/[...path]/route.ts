 ...
 *
 * Catch-all handler for OpenID Connect Discovery
 *
 ...
 * "https://example.com/.well-known/openid-configuration/oauth/test"
 *
...
export async function GET(
  ...
  if (path && path.length >= 2 && path[0] === 'oauth') {
    mcpSlug = path[1]
    console.log(`[.well-known/openid-configuration/${path.join('/')}] Extracted mcp_slug:`, mcpSlug)
  }
