...
/**
 * GET /oauth/:slug/.well-known/oauth-authorization-server
 *
 * Slug-aware OAuth 2.0 Authorization Server Metadata (RFC 8414)
 *
 * When MCP clients need to access a specific MCP server (identified by slug),
 * this endpoint returns metadata with the mcp_slug parameter included in
 * the authorization and registration endpoints.
 */
export async function GET(
  ...
  console.log(`[/oauth/${slug}/.well-known/oauth-authorization-server] Returning slug-aware metadata`)
